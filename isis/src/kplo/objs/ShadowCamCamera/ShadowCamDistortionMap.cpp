...
namespace Isis {
  ...
  /**
   * @param naifIkCode
   * optical distortial koefficient
   */
   ...
   * @see SetDistortion
   *  See Magnification and Distortion presentation 12/2021 ShadowCam Science Team Meeting
   *  
   */
  ...
  bool ShadowCamDistortionMap::SetUndistortedFocalPlane(const double ux, const double uy) {
    ...
    // To Do..This might be left over from another camera but its basically to account for more complex
    // distortion equations that might create non 1-to-1 relationship and cause extra data that should be 
    // ignored.
    // will leave as is since the value is very close to 
    // the value for shadowcam 20221012
