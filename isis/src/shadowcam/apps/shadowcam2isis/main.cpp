...
namespace Isis {
  ...
  static void TranslateShadowCamNacLabels(FileName &labelFile, Cube *ocube);
  ...
  void lronac2isis(UserInterface &ui) {
    ...
    try {
      ...
      PvlKeyword xtermKeyword = lab.findKeyword("ShadowCam:XTERM"),
                 mtermKeyword = lab.findKeyword("ShadowCam:MTERM"),
                 btermKeyword = lab.findKeyword("ShadowCam:BTERM");
    ...
    if(id.mid(13, 3) != "EDR") {
      QString msg = "Input file [" + inFile.expanded() + "] does not appear to be "
                   + "in ShadowCam EDR format. DATA_SET_ID is [" + id + "]"
    ...
    TranslateShadowCamNacLabels(inFile, g_ocube);
    p.EndProcess();
  ...
  // See "Appendix B - NAC and WAC Companding Schemes" of the ShadowCam_SOC_SPEC
  ...
  void TranslateShadowCamNacLabels(FileName &labelFile, Cube *ocube) {
